<p align="center">
  <img src="assets/banner.png" width="100%" alt="Profile Banner" />
</p>

<h1 align="center">Nishikant Pradhan</h1>
<h3 align="center">Senior QA Automation Engineer — Playwright · TypeScript · AI-Augmented Testing</h3>

<p align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=20&duration=3000&pause=1000&color=00C4FF&center=true&vCenter=true&width=650&lines=Senior+QA+Automation+Engineer;Playwright+%2B+TypeScript+%2B+BDD;US+Tax+%2F+FinTech+Domain+Expert;AI-Augmented+Testing+%7C+LLM+Integration" />
</p>

<p align="center">
  <a href="https://linkedin.com/in/nishikantpradhan">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=flat-square&logo=linkedin&logoColor=white"/>
  </a>
  <a href="mailto:nishikantpradhan1996@gmail.com">
    <img src="https://img.shields.io/badge/Email-EA4335?style=flat-square&logo=gmail&logoColor=white"/>
  </a>
  <a href="https://github.com/nishikantpradhan">
    <img src="https://img.shields.io/badge/GitHub-181717?style=flat-square&logo=github&logoColor=white"/>
  </a>
  <img src="https://komarev.com/ghpvc/?username=nishikantpradhan&style=flat-square&color=0891b2&label=Profile+Views"/>
</p>

---

### 👨‍💻 About Me

I'm a QA Automation Engineer with **6.5+ years of experience** building test frameworks and CI/CD-integrated automation for **US Tax and FinTech** platforms. My focus is on designing scalable, maintainable test architectures — and, more recently, on applying AI and LLMs to make test creation and maintenance more autonomous.

- 🎭 Deep expertise in **Playwright** across UI, API, and end-to-end workflows
- 🤖 Building **AI-augmented testing** solutions — Playwright MCP, LLM-driven test generation, agentic QA workflows
- 🥒 Strong advocate for **BDD/Cucumber** to keep QA aligned with business requirements
- ☁️ Experienced with **CI/CD pipelines**, containerized test execution, and cloud environments
- 🌱 Currently exploring **AI agents for autonomous test maintenance**
- 💬 Happy to talk about test architecture, API automation, or AI in QA

---

### 🛠️ Tech Stack

**Languages & Automation**

<img src="https://skillicons.dev/icons?i=playwright,ts,js,python"/>

**Testing & BDD**

![Cucumber](https://img.shields.io/badge/Cucumber-23D96C?style=flat-square&logo=cucumber&logoColor=white)
![Pytest](https://img.shields.io/badge/Pytest-0A9EDC?style=flat-square&logo=pytest&logoColor=white)
![Postman](https://img.shields.io/badge/Postman-FF6C37?style=flat-square&logo=postman&logoColor=white)
![REST API](https://img.shields.io/badge/REST%20API-00BCD4?style=flat-square)

**AI & Agentic Testing**

![LLM Integration](https://img.shields.io/badge/LLM%20Integration-412991?style=flat-square&logo=openai&logoColor=white)
![MCP](https://img.shields.io/badge/MCP-000000?style=flat-square)
![AI Agents](https://img.shields.io/badge/AI%20Agents-FF6F00?style=flat-square)

**Databases**

<img src="https://skillicons.dev/icons?i=mysql,mongodb"/>

**DevOps & CI/CD**

<img src="https://skillicons.dev/icons?i=git,github,githubactions,jenkins,docker,azure"/>

**Tools**

<img src="https://skillicons.dev/icons?i=vscode,linux,jira,confluence"/>

---

### 💼 Featured Projects

| Project | Description |
|---|---|
| 🎭 **Playwright + TypeScript Framework** | Scalable UI automation framework with Page Object Model and rich reporting |
| 🐍 **Playwright + Python Framework** | Cross-browser automation suite built on Python |
| 🔗 **API Automation Framework** | REST API test suite with schema and contract validation |
| 🥒 **Pytest-BDD Framework** | Gherkin-based BDD suite for business-readable scenarios |
| 🐳 **Dockerized Automation Framework** | Containerized test execution for consistent CI environments |
| ⚙️ **Jenkins CI/CD Pipeline** | Automated build-test-deploy pipeline with integrated reporting |
| 🤖 **AI-Augmented Testing (MCP)** | LLM-driven test generation using Playwright MCP and agentic workflows |

---

### 📊 GitHub Stats

<p align="center">
  <img height="165" src="https://github-readme-stats.vercel.app/api?username=nishikantpradhan&show_icons=true&theme=tokyonight&hide_border=true&count_private=true"/>
  <img height="165" src="https://github-readme-stats.vercel.app/api/top-langs/?username=nishikantpradhan&layout=compact&theme=tokyonight&hide_border=true"/>
</p>

<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=nishikantpradhan&theme=tokyonight&hide_border=true"/>
</p>

<p align="center">
  <img src="https://github-profile-trophy.vercel.app/?username=nishikantpradhan&theme=algolia&column=4&margin-w=15&margin-h=15"/>
</p>

---

### 📫 Connect With Me

<p align="center">
  <a href="https://linkedin.com/in/nishikantpradhan">
    <img src="https://img.shields.io/badge/LinkedIn-Connect-0077B5?style=for-the-badge&logo=linkedin&logoColor=white"/>
  </a>
  <a href="mailto:nishikantpradhan1996@gmail.com">
    <img src="https://img.shields.io/badge/Gmail-Email%20Me-EA4335?style=for-the-badge&logo=gmail&logoColor=white"/>
  </a>
  <a href="https://github.com/nishikantpradhan">
    <img src="https://img.shields.io/badge/GitHub-Follow-181717?style=for-the-badge&logo=github&logoColor=white"/>
  </a>
</p>

<p align="center">
  <em>"Quality is never an accident; it is always the result of intelligent effort."</em>
</p>
